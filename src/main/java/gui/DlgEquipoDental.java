...
public class DlgEquipoDental extends JDialog implements ActionListener {
	...
	void listar() {
		...
		try {
			...
			for (EquipoDental equipo : lstEquipos) {
				...
				imprimir("Fecha de adquisición	: " + equipo.getFechaAdquisicion().toLocalDate());
				...
				if (dentista != null) {
					...
					imprimir("Especialidad		: " + dentista.getEspecialidad().getTitulo());
				}
	...
	void adicionar() {
		...
		LocalDateTime fechaAdquisicion = LocalDateTime.now();
		EntityManager manager = JPAUtil.getEntityManager();

		try {

			EquipoDental equipo = new EquipoDental(null, nombre, costo, fechaAdquisicion, estado, dentista);
	...
	void buscar() {
		...
		try {
			...
			txtFechaAdquisicion.setText(equipo.getFechaAdquisicion().toLocalDate().toString());
	...
	void modificar() {
		...
		String estado = cboEstados.getSelectedItem().toString();
		Dentista dentista = (Dentista) cboDentistas.getSelectedItem();
		LocalDateTime fecha = LocalDateTime.parse(txtFechaAdquisicion.getText() + "T00:00:00");
		EntityManager manager = JPAUtil.getEntityManager();

		try {

			EquipoDental equipo = new EquipoDental(nroEquipo, nombre, costo, fecha, estado, dentista);
